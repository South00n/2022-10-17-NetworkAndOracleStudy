...
-- 오라클에 저장
-- user_table, user_views, user_constraints
...
-- 수정이 가능하게 만든다
-- 사용빈도가 거의 없다 (단순뷰 => 테이블 한개 연결)
CREATE OR REPLACE VIEW dept_view
AS
SELECT empno, ename, job, hiredate, sal FROM emp;
-- 뷰는 테이블과 동일시 된다 => 함수, 연산자 사용이 가능 => 많이 사용되는 SQL문장이 있는 경우 => VIEW생성
-- 복합뷰 (테이블 여러개 연결해서 사용)
...
/*
     인라인뷰 => 뷰를 생성하는 것이 아니라 => SELECT를 이용하는 방식 => 테이블에서 몇개를 잘라서 출력
                                         페이지를 나눠서 출력
                                         => 단점 : Top-N
     형식)
          SELECT ~~
...
/*
      ...
      SQL문장을 자주 사용한다(같은 거) => 재사용을 위해 뷰를 제작해서 사용 ==> 거의 사용안함 (MyBatis => 중복처리)
      ----------------------------------------------------- 뷰
*/
