/*
    ...
    *** 247page
        1) 자료를 쉽게 빠르게 찾을 수 있게 만든 데이터 구조
        2) B-tree
            루트      -----
                       |
            ---------------------------------  내부 루트
...
SELECT * FROM emp WHERE sal > 0; -- 0보다 큰값에 대해서 정렬해라
...
-- 입문 : DML, TCL, VIEWM, SEQUENCE => INDEX/PL~SQL
/*
       1) 인덱스 사용 목적
          => 쉽고 빠르게 검색 결과를 가지고 올 수 있게 만든 구조(B tree)
          => SQL명령문의 처리 속도를 향상
          => 최적화
          => 검색어로 많이 사용되는 컬럼이 있는 경우 (영화제목, 영화출연, 영화 장르)
          => JOIN에서 비교되는 컬럼
