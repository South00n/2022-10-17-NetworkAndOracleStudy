 ...
 * 		  -------- 서울시청, 안산시청, 수원시청
 * 				   ------  --------------
 * 				    오라클 	   MSSQL	--> 같은 인터페이스를 사용하기때문에 추후에 바꿀때 편해짐 (호환성 유지)
 ...
 * 		  }
 * 		  내부클래스
 * 		  = 멤버클래스 (클래스의 내용(변수, 메소드)을 공유) ==> 안쓰면 static을 이용한다 ==> 쓰레드, 윈도우, 서버구축
 * 			class A {
 * 				공유할 데이터
 * 				공유할 메소드
 * 				class B {
 * 					A가 가지고 있는 모든 변수, 메소드를 사용할 수 있다 (private도 사용가능)
 * 				}
 * 			}
 ...
 * 			}
 * 			
 * 		  종단클래스 => 사용자 정의는 거의 없다 => java.lang.* (대부분 종단 클래스)
...
public class WaitRoom extends JPanel {
	...
	public WaitRoom() {
		...
		String[][] row1 = new String[0][3]; // 한줄에 데이터 3개
		...
		String[][] row2 = new String[0][3]; // 한줄에 데이터 3개
		model2 = new DefaultTableModel(row2, col2) { // 익명의 클래스
