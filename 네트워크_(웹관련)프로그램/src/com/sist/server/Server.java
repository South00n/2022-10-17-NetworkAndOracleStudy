...
public class Server implements Runnable {
	 ...
	 *    	  Map => 관리 : HashMap
	 *    
	 *    	  제네릭스<클래스형> => Wrapper클래스 공부
	 *    	  ------ 데이터형을 변경 (Object -> 지정된 데이터형으로 변경)
	 *    	  ------ 형변환을 하지않는다 ==> 소스가 간결하다 ==> 가독성(어떤 데이터형을 첨부했는지 확인)
	...
	public Server() {
		try {
			ss = new ServerSocket(PORT); // bind() : 소켓생성, listen() : 대기상태 -> 자동호출
	...
	class Client extends Thread {
				...
				while(true) {
					...
					switch (protocol) {
						case 100: { // 로그인이면
							...
							messgeAll(100 + "|" + id + "|" + name + "|" + sex);
							// 2.watiVc에 저장
							waitVc.add(this);
							// 2-2.로그인창에서 대기실창으로 변경
							messageTo(110 + "|" + id);
							// 3.로그인 한 사람에게 => 접속자 정보를 전체 전송
						...
						case 900 : { // 나가기면...
							...
							messgeAll(900 + "|" + id); // 테이블에서 id를 제거 (남아 있는 사람처리)
							// 제거
							for(int i = 0; i <waitVc.size(); i++) {
								Client c = waitVc.get(i);
								if(c.id.equals(mid)) {
									messageTo(990 + "|"); // 윈도우를 종료한다
		...
		// 전체적으로 전송
		public synchronized void messgeAll(String msg) {
			try {
				for(Client client : waitVc) {
					client.messageTo(msg); // 대기실이나 채팅
				}
			} catch (Exception ex) {}
		}
